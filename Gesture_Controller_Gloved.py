...
class Marker:
    ...
    def detect(self, frame):
        ...
        self.corners, ids, rejectedImgPoints = aruco.detectMarkers(gray_frame, self.aruco_dict,
...
class ROI:
    ...
    def find_glove_hsv(self, frame, marker):
        ...
        bot_ry = int(self.marker_top[1] - self.hsv_lift_up * l + self.hsv_alpha * slope_12 * l * np.sqrt(
            1 / (1 + slope_12 ** 2)))

        bot_lx = int(self.marker_top[0] - self.hsv_alpha * l * np.sqrt(1 / (1 + slope_12 ** 2)))
        bot_ly = int(self.marker_top[1] - self.hsv_lift_up * l - self.hsv_alpha * slope_12 * l * np.sqrt(
            1 / (1 + slope_12 ** 2)))
...
class Tracker:
    ...
    def CSRT_tracker(self, frame):
        ...
        if self.now_time - self.start_time >= 2.0:
            ...
            cv2.putText(frame, 'Posture your hand correctly', (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 1,
                        cv2.LINE_AA)
